...
from reportlog.rem_days_file import RemoveDaysFile
...
def backup_table(log_type,tb):
    ...
    try:
        ...
        tb_csv = BAK_PATH + log_type + '/' + tb + '.csv'
        export_table_csv(tb, tb_csv)
        # if os.path.exists(tb_sql):
        #     os.remove(tb_sql)
        #     logger.debug('file %s is exists, removed' % tb_sql)
        #     FWLOG_DEBUG('file %s is exists, removed' % tb_sql)
        #     pass
...
def mysql_log_recover_by_date(types,date):
    """
        Description:
            ...
            def import_csv_table(csvfile, tb, tb_type):
    """
    ...
    tb_csv = BAK_PATH + types + '/' + types + '_' + date + '.csv'
    ...
    if not os.path.exists(tb_csv):
        ...
        # FWLOG_DEBUG('FILE %s is NOT EXISTS!!!' % tb_sql)
        logger.debug('FILE %s is NOT EXISTS!!!' % tb_csv)
        FWLOG_DEBUG('FILE %s is NOT EXISTS!!!' % tb_csv)
        return -1

    # recover table from types_date.sql
    # logger.debug('RECOVER File:%s' % tb_sql)
    # FWLOG_DEBUG('RECOVER File:%s' % tb_sql)
    logger.debug('RECOVER File:%s' % tb_csv)
    FWLOG_DEBUG('RECOVER File:%s' % tb_csv)
    try:
        # cmd = ("/usr/bin/mysql --port=3307 --socket=/tmp/mysql3307.sock "
        #        "-uroot --password='bd_123456' --port=3307  -e 'source %s'")
        # os.system(cmd % tb_sql)
        import_csv_table(tb_csv, types + '_' + date, types)
        return 1
    except Exception as e:
        FWLOG_DEBUG(e)
...
def old_enough_to_backup(newest,this):
    """
        Description:
            ...
            OUTPUT:
                compare newst date and 'this', return if 'this' is 30(ten) days
                older than newst
    """
    ...
    # KEEP 30 DAYS
    #30 DAYS = 60(s) * 60(min) * 24(h) * 30(d) = 2592000(s)
    THIRTY_DAYS = 2592000
    return (ts_newest - ts_this) > THIRTY_DAYS
...
def mysql_log_delete_old_tb(name):
    ...
    if delete_list != []:
        ...
        FWLOG_DEBUG('BACKUP Before DELETE:%s' % delete_list)
...
def export_table_csv(tb, _path):
    ...
    if os.path.exists(_path):
        FWLOG_DEBUG('%s is already exists...' % _path)
        return
    FWLOG_DEBUG('backup {table} to {path}'.format(table=tb, path=_path))
...
def import_csv_table(csvfile, tb, tb_type):
    """
        ...
        LINES TERMINATED BY '\n'"
    """
    import_sql = ('LOAD DATA INFILE "{path}" INTO TABLE `{table}` '
                  'FIELDS TERMINATED BY "," '
                  'LINES TERMINATED BY "\n"')

    create_sql = "CREATE TABLE IF NOT EXISTS `{tb}` LIKE {tb_type}".format(tb=tb,
                                                                           tb_type=tb_type)
    execute_sql(create_sql)
    FWLOG_DEBUG(create_sql)
    execute_sql(import_sql.format(path=csvfile, table=tb))
    pass

def remove_old_backup_csv():
    # remove sql/csv files which are too old
    days = '60days'
    path = '/var/log/log_tables_backup'
    rem = RemoveDaysFile()
...
if __name__ == '__main__':
    ...
    # KEEP THIS ALIVE!!!!!
    everyday_task()
    remove_old_backup_csv()


    #-------------------TEST-------------------
    # export_table_csv('m_tblog_ddos_20161012')
    # import_csv_table('/tmp/m_tblog_ddos_20161012.bak', 'm_tblog_ddos_20161013', 'm_tblog_ddos')
